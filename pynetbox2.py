...
FK_FIELDS = {
    "dcim.devices": ["site", "tenant", "role", "device_type", "platform", "location", "rack"],
    "virtualization.virtual_machines": ["cluster", "tenant", "role", "platform"],
    "virtualization.clusters": ["group", "type", "site"],
    "ipam.vlans": ["site", "tenant", "group"],
    "ipam.prefixes": ["site", "tenant", "vlan", "vrf"],
    "ipam.ip_addresses": ["vrf", "tenant", "interface", "assigned_object"],
    # Add more as needed
}
...
class NetBoxExtendedClient:
    ...
    def _normalize_for_compare(self, value: Any) -> Any:
        if isinstance(value, dict):
            return {k: self._normalize_for_compare(v) for k, v in sorted(value.items())}
        if isinstance(value, (list, tuple, set)):
            return [self._normalize_for_compare(v) for v in value]
        if hasattr(value, "id"):
            return getattr(value, "id")
        return value
        ...
        *,
        ...
        for field in lookup_fields:
            if field not in payload:
                continue
            value = payload[field]
            # Use the same normalization as for payload keys
